...
public class StateRun extends GameState {
    ...
    public void initialize(Map<String, Object> data) {
        ...
        //  太陽數量
        _NumberOfSun = new Integer(DEFAULT_SCORE_DIGITS, 1000 , 310 , 15 );
        ...
        //  計算殭屍出現時間
        ZombieAppearTimes = 0 ;
        _music = new Audio(R.raw.piano_store);
    ...
    public void move() {
        ...
        if( NextLevel ){
            ...
            if( LevelState == 5 ){
                LevelState = 1 ;
                changeState(Game.Level_2_Ready);
            }
            else changeState( Game.Level_1_Ready );
        }
        if( GameOver ){
            LevelState = 1 ;
    ...
    public boolean pointerPressed(List<Pointer> pointers) {
        if (pointers.size() == 1) {
            ...
            if( _OnTouchPlant > 0 ){
                ...
                else if( _NumberOfSun.getValue() < cost && _gameMap.PlantInSpace( BlockPosition[0] , BlockPosition [1]  ) == 0 ){
                    //  顯示警告，太陽數目不夠
                    _Warning.ChangeWarning( 1 );
                }
            }
