...
import tw.edu.ntut.csie.game.GameMap_Water;
...
import tw.edu.ntut.csie.game.extend.*;
...
public class StateRun2 extends GameState {
    ...
    public void move() {
        ...
        if( NextLevel ){
            ...
            if( LevelState == 5 ){
                LevelState = 1 ;
                changeState(Game.Level_3_Ready);
            }
            else changeState( Game.Level_2_Ready );
        }
        if( GameOver ){
            LevelState = 1 ;
    ...
    public void show() {
        ...
        for( number_i = 0 ; number_i < MapRow ; number_i++ ){
            for( number_j = 0 ; number_j < MapColumn ; number_j++ ){
                if( number_i > 0 && number_i < MapRow - 1 ) _DataPlant[number_i][number_j].ShowLotus();
    ...
    public boolean pointerPressed(List<Pointer> pointers) {
        if (pointers.size() == 1) {
            ...
            if( _OnTouchPlant > 0 ){
                ...
                if( _gameMap.InRange( touchX , touchY ) == 1
                        && _gameMap.WaterSpaceCanGrow(  touchX , touchY , BlockPosition , _OnTouchPlant , _Warning ) == 1
            ...
            if( _OnTouchShovel){
                int [] BlockPosition = { 0 , 0 } ;
                if( _gameMap.InRange( touchX , touchY ) == 1 && _gameMap.WaterSpaceCanDig( touchX , touchY , BlockPosition ) == 1 ){
            ...
            //  撿拾掉落中的太陽
            if( _FallSunFlower.IsProducing() ){
                int tmp ;
                tmp = _FallSunFlower.TouchSun( touchX , touchY ) ;
                if( tmp == 1 ){
                    _NumberOfSun.add( 25 );
                }
            }
            ...
            for( number_i = 0 ; number_i < MapRow ; number_i ++ ){
                for( number_j = 0 ; number_j < MapColumn ; number_j ++ ){
                    if( _DataPlant[ number_i ][ number_j ].TouchSun( touchX , touchY ) == 1 ) _NumberOfSun.add( 25 );
                }
            }
        }
        return true;
